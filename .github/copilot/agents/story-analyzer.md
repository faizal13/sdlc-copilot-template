...
- If GitHub MCP is unavailable → output a bash script to save the issue as a local file

> **Platform note:** GitHub Copilot custom agents cannot write files via the `codebase` tool.
> For local mode, output a bash script — VS Code shows **"Run in Terminal"** on bash code blocks.

**Local mode — output this bash script (with real content replacing all placeholders):**

~~~bash
#!/bin/bash
# Generated by @story-analyzer — {ADO_ID} GitHub Issue draft (local mode)
# GitHub MCP unavailable — click "Run in Terminal" to save locally

mkdir -p docs/issues

cat > docs/issues/{ADO_ID}-{service-name}-issue.md << 'ISSUE_EOF'
{full GitHub Issue markdown content — same structure as if creating a real issue}
ISSUE_EOF
echo "✅ Saved: docs/issues/{ADO_ID}-{service-name}-issue.md"
echo "When GitHub is available: create issue from this file and add label 'ai-generated'"
~~~

Create the single GitHub Issue (or run the bash script above), then output:
