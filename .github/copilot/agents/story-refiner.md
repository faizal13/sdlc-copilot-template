...
Output the following bash script (with real content replacing all `{placeholders}`):

~~~bash
#!/bin/bash
# Generated by @story-refiner — save EPIC-{id} execution plan
# Click "Run in Terminal" in VSCode to create the file

mkdir -p docs/epic-plans

cat > docs/epic-plans/EPIC-{id}-execution-plan.md << 'PLAN_EOF'
...
PLAN_EOF
echo "✅ Created: docs/epic-plans/EPIC-{id}-execution-plan.md"
~~~

> Fill ALL `{placeholders}` with real content before outputting.
