...
> (it is read-only for custom agents). Output a bash script instead — VS Code shows a
> **"Run in Terminal"** button on bash code blocks. Developer clicks once, file is on disk.
>
> Output the following bash script with ALL `{placeholders}` replaced with real content.
> The closing `PLAN_EOF` must be on its own line with NO leading spaces.

~~~bash
#!/bin/bash
# Generated by @task-planner — {STORY-ID} task plan
# Click "Run in Terminal" in VSCode to create the file

mkdir -p taskPlan

cat > taskPlan/{filename}.md << 'PLAN_EOF'
...
PLAN_EOF
echo "✅ Created: taskPlan/{filename}.md"
~~~

---

## Step 6 — Validate Before Outputting Script
...
After outputting the bash script, show:

```
▶  Click "Run in Terminal" on the script above to save the task plan to disk.
